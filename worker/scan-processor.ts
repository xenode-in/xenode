...
export const createScanWorker = () => {
  return new Worker(
    ...
    async (job: Job<ScanJobData>) => {
      ...
      const foldersToScan = folderId.includes(',') ? folderId.split(',') : [folderId];

      for (const targetFolderId of foldersToScan) {
        ...
        if (targetFolderId !== "root" && !baseFolderPath) {
          try {
            const folderMeta = await adapter.getFileMetadata(targetFolderId);
            baseFolderPath = folderMeta.name;
          } catch (err) {
            console.warn(`Could not fetch metadata for folder ${targetFolderId}`, err);
          }
        }

        let pageToken: string | undefined;
        do {
          const result = await adapter.listFiles(targetFolderId, pageToken);
