 ...
 * are intentionally separate from the v1 cell/byte limits: the v2 pipeline holds
...
export const MAX_WORKBOOK_BYTES = num(
  process.env.NEXT_PUBLIC_SHEETS_V2_MAX_BYTES,
  75 * 1024 * 1024,
);
...
export const MAX_EDITOR_BIN_BYTES = num(
  process.env.NEXT_PUBLIC_SHEETS_V2_MAX_BIN_BYTES,
  300 * 1024 * 1024,
);
...
export const MAX_BRIDGE_PAYLOAD_BYTES = num(
  process.env.NEXT_PUBLIC_SHEETS_V2_MAX_BRIDGE_BYTES,
...
export function assertWorkbookSize(byteLength: number): void {
  if (byteLength > MAX_WORKBOOK_BYTES) {
    throw new WorkbookLimitError(
      `Workbook exceeds the ${Math.round(MAX_WORKBOOK_BYTES / 1024 / 1024)} MB v2 safety limit.`,
      "workbook_too_large",
    );
  }
}
...
export function assertEditorBinSize(byteLength: number): void {
  if (byteLength > MAX_EDITOR_BIN_BYTES) {
    throw new WorkbookLimitError(
      `Converted document exceeds the ${Math.round(MAX_EDITOR_BIN_BYTES / 1024 / 1024)} MB v2 safety limit.`,
