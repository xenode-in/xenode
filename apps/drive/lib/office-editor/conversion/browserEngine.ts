/**
 * lib/office-editor/conversion/browserEngine.ts
 *
 ...
 * throws `X2tUnavailableError`, so callers can fail closed without parsing the
 * workbook on the application origin.
 */
...
const MAX_FONTS = Number(process.env.NEXT_PUBLIC_OFFICE_EDITOR_MAX_FONTS) || 60;
...
async function loadFonts(mod: RawX2tModule): Promise<void> {
  ...
  try {
    ...
    console.warn("[office-editor] font manifest unavailable; x2t may substitute fonts", err);
    return;
  }
