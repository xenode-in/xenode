  ...
  try {
    ...
    if (!bucket) {
      try {
        ...
        } else if (err.Code === "BucketAlreadyExists" || err.name === "BucketAlreadyExists") {
          return NextResponse.json(
            { error: `Storage bucket '${GLOBAL_BUCKET_NAME}' is already taken. Please set a unique B2_BUCKET_NAME.` },
            { status: 500 },
          );
        } else {
          return NextResponse.json({ error: "Failed to initialize storage: " + err.message }, { status: 500 });
        }
      }
    }

    return NextResponse.json({ bucket, rootPrefix: `users/${session.user.id}/` });
  } catch (error: any) {
