...
function SheetsEditorInner() {
  ...
  const { data: session } = useSession(); const { privateKey, metadataKey } = useCrypto(); const workspaceContext = useWorkspace(); const space = useWorkspaceSpaceKey(); const { addTasks } = useUpload();
  ...
  }, [metadataKey, privateKey, space.cryptoKey, space.rawSpaceKey, workspace, workspaceContext.scopedFetch]);
  useEffect(() => {
    if (!persistence || !objectId || !session?.user.id) return; const controller = new AbortController();
    // Matches the repository client-detail loading convention.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setLoaded(null); setError(null);
    persistence.load(objectId, controller.signal).then(async (value) => { setLoaded(value); await getDb(session.user.id).spreadsheetRecents.put({ id: `${workspace.workspaceId}:${objectId}`, userId: session.user.id, objectId, workspaceId: workspace.workspaceId, organizationId: workspace.type === "personal" ? undefined : workspace.organizationId, lastOpenedAt: Date.now() }); }).catch((reason) => { if (reason?.name !== "AbortError") setError(reason instanceof Error ? reason.message : "spreadsheet_load_failed"); });
    return () => { controller.abort(); persistence.dispose(); };
  }, [objectId, persistence, reloadKey, session?.user.id, workspace]);
  const reload = useCallback(() => setReloadKey((value) => value + 1), []);
  if (!objectId) return <div className="flex h-full items-center justify-center text-sm">No spreadsheet object was selected.</div>;
  if (error) return <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center"><p className="font-medium">Unable to open spreadsheet</p><p className="text-sm text-muted-foreground">{error}</p><button className="text-sm underline" onClick={() => router.push("/sheets")}>Back to spreadsheets</button></div>;
  if (!loaded || !persistence || !session?.user.id || !metadataKey) return <SpreadsheetLoadingState/>;
  const recoveryKey = workspace.type === "personal" ? metadataKey : space.cryptoKey!;
  return <SpreadsheetEditor loaded={loaded} persistence={persistence} userId={session.user.id} recoveryKey={recoveryKey} onReload={reload} onBack={() => router.push("/sheets")} onSaveCopy={bucketId && prefix ? (file) => addTasks([new File([file], file.name.replace(/\.xlsx$/i, " copy.xlsx"), { type: file.type })], bucketId, prefix) : undefined}/>;
}
