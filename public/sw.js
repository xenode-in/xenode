...
self.addEventListener('message', async (e) => {
  if (e.data?.type !== 'REGISTER_STREAM') return;

  const { fileId, rawDEK, chunkSize, chunkCount, chunkIvs, urls, contentType, size } = e.data;
  ...
  streamRegistry.set(fileId, { dek, chunkSize, chunkCount, chunkIvs, urls, contentType, plainSize });
...
self.addEventListener('fetch', (e) => {
  ...
  if (!config) {
    e.respondWith(
      new Response('Stream session not found (SW reset). Please reload.', { status: 404 }),
    );
    return;
  }
...
async function buildResponse(config, request) {
  const { dek, chunkSize, chunkCount, chunkIvs, urls, contentType, plainSize } = config;
  ...
  const totalChunksNeeded = endChunk - startChunk + 1;
  ...
  for (let i = startChunk; i < Math.min(startChunk + PREFETCH, chunkCount); i++) {
